...
def detect_corneres(raw_img):
    ...
    ract = np.int32(normalize_corners(rect)* scale)
...
def fit_size(img, h, w):
    ...
    return cv2.resize(img, (int(size[f]*f), int(size[0]*f)), interpolation=cv2.INTER_AREA)
...
def contours(img, show=True):
    ...
    min_area = img.shape[0] * img.shape[1]
...
def get_get_fit_score(img, x):
    ...
    kernel = np.zeros((int(img_size / 10), int(img_size /10 )), np.uint8)
